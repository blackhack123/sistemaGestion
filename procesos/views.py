    ...
    if 'nombreUsuario' not in request.session:
        ...
        if proceso_existente == 0:
            proceso = Procesos(
                ...
                url_carpeta=smart_str(request.POST['destino']) + '/' +smart_str(request.POST['directorio'])
            )
            proceso.save()
            #creando la carpeta de destino
            try:
                os.makedirs(smart_str(request.POST['destino']) + "/" + smart_str(request.POST['directorio']), mode=0777)
            except OSError:
                messages.add_message(request, messages.WARNING, 'Error - nose creo el Directorio')
...
def updateProceso(request):
    ...
    if 'nombreUsuario' not in request.session:
        ...
        #renombrar carpeta
        carpeta_anterior = smart_str(request.POST.get('carpeta_anterior'))
        nuevo_nombre = smart_str(request.POST.get('proceso'))
        if carpeta_anterior != nuevo_nombre:
            try:
                path = 'media/gestionDocumental/'
                path_completo = smart_str(request.POST.get('destino'))
                for root, dirs, files in os.walk(path):
                    for directory in dirs:
                        if directory == carpeta_anterior:
                            os.rename(smart_str(path_completo+'/'+carpeta_anterior), smart_str(path_completo+'/'+nuevo_nombre))
                            messages.add_message(request, messages.SUCCESS, 'Registro actualizado existosamente !!')
                            return redirect('procesos')
            except OSError:
                messages.add_message(request, messages.WARNING, 'Error - nose actualizo el directorio')
                return redirect('procesos')
        else:
            messages.add_message(request, messages.SUCCESS, 'Registro actualizado existosamente !!')
            return redirect('procesos')
...
def gridVinculados(request):
    ...
    if 'nombreUsuario' not in request.session:
        ...
        query = query + " ORDER BY areas_areas.area, areas_procesos.proceso "
        cursor.execute(query)
        rows = dictfetchall(cursor)
        # CONVIERTE  QuerySet  a list object
        procesos_list = list(rows) 
